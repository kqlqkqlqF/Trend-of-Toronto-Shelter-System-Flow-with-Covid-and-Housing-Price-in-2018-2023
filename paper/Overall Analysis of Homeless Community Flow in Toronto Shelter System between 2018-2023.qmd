...
# Change column names
colnames(data) <- c("Year", "Returned from Housing", "Returned to Shelter", 
                    "Newly Identified", "Moved to Housing", "Became Inactive", 
                    "Actively Homeless")

# Create a kable table
kable(cleaned_data_figone, align = "c", caption = "Shelter System Flow from 2018 to 2023") %>%
...
####2nd graph data####
# Step 1: Filter out unnecessary population groups
cleaned_data_figtwo <- cleaned_data_figtwo %>%
  filter(population_group %in% c("All Population", "Chronic", "Refugees", "Families", "Youth", "Single Adult", "	
Non-refugees"))
...
# Define a custom Starry Night color palette
starry_night_palette <- c("#145b7d", "#70a19f", "#008792","#00ae9d", "#76becc", "#94d6da")

# Plot the data with thinner bars
ggplot(reshaped_data, aes(x = age_group, y = count, fill = population_group)) +
  geom_bar(stat = "identity", position = position_dodge(width = 0.8), width = 0.8) +  # Adjust the width here
...
ggplot(cleaned_data_figthree_long, aes(x = year, y = count, fill = gender)) +
  ...
  theme(axis.text.x = element_text(angle = 45, hjust = 1),  # Rotate x-axis labels
        ...
        legend.box.spacing = unit(0.3, "lines"),  # Reduce spacing around legend
        legend.spacing = unit(0.3, "lines"))  # Reduce spacing within legend
```

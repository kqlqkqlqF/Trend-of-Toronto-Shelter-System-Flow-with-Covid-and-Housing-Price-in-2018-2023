...
\[
\text{All Population}_i \sim \mathcal{N}(\mu_i, \sigma)
\]
\[
\mu_i = \beta_0 + \beta_1 \times \text{DATE}_i
\]
\[
\beta_0 \sim \mathcal{N}(\mu = \text{Initial Estimate}, \sigma = \text{Standard Deviation})
\]
\[
\beta_1 \sim \mathcal{N}(\mu = \text{Initial Estimate}, \sigma = \text{Standard Deviation})
\]
...
\textbf{Intercept ($\beta_0$)}: The intercept represents the estimated number of the homeless population at the beginning of the observation period, which is 2018 January.

\textbf{Slope ($\beta_1$)}: The slope quantifies the average change in the number of the homeless population per month.
...
The equation of the linear model is as follows:
\[
\text{All Population}_i = \beta_0 + \beta_1 \times \text{DATE}_i
\]

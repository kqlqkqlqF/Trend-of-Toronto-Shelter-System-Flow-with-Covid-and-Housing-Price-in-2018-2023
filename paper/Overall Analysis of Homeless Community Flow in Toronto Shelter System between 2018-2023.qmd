...
# Convert 'year' to a factor for proper ordering in the bar chart

cleaned_data_figone$year <- as.factor(cleaned_data_figone$year)

# Plotting with ggplot2

ggplot(cleaned_data_figone, aes(x = year)) +

  geom_bar(aes(y = returned_from_housing), stat = "identity", fill = 'blue') +

  geom_bar(aes(y = returned_to_shelter), stat = "identity", fill = 'red', position = 'stack') +

  geom_bar(aes(y = newly_identified), stat = "identity", fill = 'green', position = 'stack') +

  geom_bar(aes(y = moved_to_housing), stat = "identity", fill = 'orange', position = 'stack') +

  geom_bar(aes(y = became_inactive), stat = "identity", fill = 'purple', position = 'stack') +

  geom_bar(aes(y = actively_homeless), stat = "identity", fill = 'yellow', position = 'stack') +

  labs(x = 'Year', y = 'Number of People', fill = 'Category', title = 'Homelessness Data by Year and Category') +
